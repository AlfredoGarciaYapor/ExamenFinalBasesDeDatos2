                ...
                <b-col lg="3" class="my-1">
                  <b-form-group label="Filtro" label-cols-sm="3" label-align-sm="right" label-size="sm" label-for="filterInput" class="mb-4">
                    <b-input-group size="sm ms-3">
                      <b-form-input v-model="filter" class="me-2" type="search" id="filterInput" placeholder="Escribe para buscar" style="border-radius: 20px"></b-form-input>
                      <b-input-group-append>
                        <b-button class="btn-limpiar ms-2 me-4" style="border-radius: 20px" @click="filter = ''" >Limpiar</b-button>
                      </b-input-group-append>
                    </b-input-group>
                  </b-form-group>
                </b-col>
            </b-row>
             <b-table responsive class="mx-3" show-empty stacked="md" small hover :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage" :filter="filter" :filter-included-fields="filterOn"   @filtered="onFiltered"><!-- :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" -->
...
export default {
    ...
    data() {
        return {
            items:[],
            fields:[
                { key: "name", label: "Nombre", class: "text-left"},
                { key: "alpha3Code", label: "Código de tres letras", sortDirection: "desc" },
                { key: "capital",  label: "Cápital",   class: "text-left" },
                { key: "region", label: "Continente", class: "text-left"},
                { key: "subregion", label: "Región", class: "text-left" },
                { key: "actions", label: "" },
            ],
